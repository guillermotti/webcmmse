    ...
    this.firebaseService
      ...
      .then(() => {
        this.translationService
          .get('_USER_PAYMENT_UPDATED_SUCCESFULLY')
          .subscribe(resp => {
...
export class ConfirmPaymentDialogComponent {
  ...
  changeStatusPayment(user) {
    const observable = this.firebaseService
      .getCollection('config')
      .subscribe(response => {
        if (
          user.check_payment === true &&
          user.bill &&
          _.isNil(user.bill.invoice_number)
        ) {
          user.bill.invoice_number = response[0].bill_number;
          response[0].bill_number = response[0].bill_number + 1;
          this.firebaseService.updateItemFromCollection(
            'config',
            response[0].id,
            response[0]
          );
