      ...
      .subscribe(users => {
        users.forEach(user => {
          const userToExport = [
            ...
            user.address,
            user.phone,
            user.check_payment ? 'Yes' : 'No',
            user.payment_file ? user.payment_file.url_file : '',
            user.bill ? user.bill.invoice_number : '',
            user.bill ? user.bill.CIF : ''
          ];
          data.push(userToExport);
        });
        /* generate worksheet */
        ...
        papers.forEach((paper, index) => {
          doc.fromHTML(htmlImages, 0, 0, {}, function() {
            ...
            doc.text(
              20,
              100,
              usersOfPapers[index].title +
                ' ' +
                usersOfPapers[index].first_name +
                ' ' +
                usersOfPapers[index].last_name
            );
            doc.setFontType('normal');
            doc.text(
              20,
    ...
    const obser = this.firebaseService
      ...
      .subscribe(users => {
        ...
        usersTotal.forEach((user, index) => {
          const htmlImages = `<div><img src="../../../assets/img/CMMSE_VertIzq.jpg" style="position:absolute;
            ...
            if (user[1]) {
              ...
              if (_this.text) {
                ...
                doc.text(
                  infoWidth,
                  150,
                  user[1].last_name + ', ' + user[1].first_name,
                  null,
                  null,
                  'center'
                );
            ...
            if (user[2]) {
              ...
              if (_this.text) {
                ...
                doc.text(
                  infoWidth,
                  240,
                  user[2].last_name + ', ' + user[2].first_name,
                  null,
                  null,
                  'center'
                );
